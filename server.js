...
// SEARCH FUNCTION — FINAL BULLETPROOF VERSION (replaces everything below)
function searchKnowledgeBase(query, limit = 5) {
  ...
  const scored = knowledgeBase.qaDatabase.map(qa => {
    ...
    // Any exact keyword match in query = instant high score
    const keywordMatch = qa.keywords.some(k => queryLower.includes(k.toLowerCase()));
    if (keywordMatch) score += 13;   // guaranteed trigger

    // Question substring match
    if (queryLower.includes(qa.question.toLowerCase().substring(0, 20))) score += 5;

    // Word-by-word overlap in question/answer/keywords
